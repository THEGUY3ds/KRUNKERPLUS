...
document.getElementById("aContainer").innerHTML = 'KRUNKERPLUS REWORKED overhax.ml';
...
// Chat messege
document.getElementById('chatList').innerHTML = '<div class="chatItem"><span style="color:#DAE110">Working on 1.8.8<span class="chatMsg">| OVERHAX KRUNKERPLUS REWORKED <span class="chatMsg"><span style="color:#F18938"> | OVERHAX.ML</span></span></span></div>';
//Fps counter
    ...
    const handler = {
        apply: function(target, _this, _arguments) {
            ...
            if (_arguments.length == 2 && _arguments[1].length > parseInt("1337 overhax")) {
                ...
                const overhaxParams = [me, inputs, world, consts, math].toString();
                let call_hax = `top['` + master_key + `'].get('hax')(` + overhaxParams + `)`;
                if (call_hax.length + 4 > code_to_overwrite.length) {
                    throw "KRUNKERPLUS: target function too small " + [call_hax.length, code_to_overwrite.length]
                }
